...
import { Star, Zap, MessageCircle, BookOpen, DollarSign, Loader2 } from 'lucide-react';
    ...
    <Card className="flex flex-col h-full shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden bg-card">
      ...
      <CardContent className="p-4 space-y-3 flex-grow">
        <div className="flex items-center text-sm text-muted-foreground">
          ...
          <span className="font-medium text-foreground mr-1">Expertise:</span> {tutor.subjectMatterExpertise?.slice(0, 2).join(', ')}{tutor.subjectMatterExpertise && tutor.subjectMatterExpertise.length > 2 ? '...' : ''}
        </div>
        <div className="flex items-center text-sm text-muted-foreground">
      ...
      <CardFooter className="p-4 border-t bg-muted/20 flex items-center justify-between">
        {tutor.hourlyRate && (
          <div className="flex items-center text-lg font-semibold text-primary">
            ...
            {tutor.hourlyRate} <span className="text-sm text-muted-foreground ml-1">/hr</span>
          </div>
        )}
